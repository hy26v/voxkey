# ABOUTME: Exercises lazy, bounded Parakeet initialization and transcription with a real model.
# ABOUTME: Keeps model files, audio, D-Bus, EIS, and injected text inside the isolated VM stack.
...
def daemon_config(isolated_voxkey_home):
    source = os.environ.get("VOXKEY_TEST_PARAKEET_MODEL_DIR")
    if not source:
        pytest.skip("set VOXKEY_TEST_PARAKEET_MODEL_DIR for the real-model run")
    source = Path(source).resolve()
    if not all((source / name).is_file() for name in REQUIRED_MODEL_FILES):
        pytest.skip("the configured Parakeet model is incomplete")
...
@pytest.mark.asyncio
async def test_real_parakeet_initializes_and_transcribes_within_bounds(
    daemon_process,
    dbus_session,
    virtual_mic,
    fixtures_dir,
    ...
    try:
        ...
        live = await wait_until(
            daemon.get_live_transcript, lambda text: bool(text.strip()), timeout=60,
        )
        preview_latency = time.monotonic() - playback_started
        assert live.strip(), "the bounded recognizer produced no live preview"
        assert preview_latency < 60, f"first preview took {preview_latency:.2f}s"
    ...
    final = await asyncio.wait_for(completed, timeout=60)
    reference = (
